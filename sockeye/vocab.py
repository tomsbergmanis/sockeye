...
def build_from_paths(paths: Iterable[str], is_alignment: bool = False, num_words: Optional[int] = None,
    ...
    :param is_alignment: Files are JSON alignment that contain one dictionary per line.
...
def count_tokens(data: Iterable[str], is_alignment: bool = False) -> Counter:
    ...
    :param data: Sequence of sentences containing whitespace-delimited tokens.
    :param is_alignment: Data is JSON alignment that contains one dictionary per
...
def load_or_create_vocabs(shard_source_paths: Iterable[Iterable[str]],
    ...
    if shared_vocab:
        ...
        vocab_source = load_or_create_vocab(shard_source_sentence_paths, source_vocab_path, num_words=num_words_source,
                                            word_min_count=word_min_count_source, pad_to_multiple_of=pad_to_multiple_of,
                                            mapper=mapper)
        vocab_target = load_or_create_vocab(shard_target_sentence_paths, target_vocab_path, num_words=num_words_target,
                                            word_min_count=word_min_count_target, pad_to_multiple_of=pad_to_multiple_of,
                                            mapper=mapper)
